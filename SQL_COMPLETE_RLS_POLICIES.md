...
-- Drop existing policies for customers (if any)
...
**Expected**: All tables should have `rowsecurity = true` and multiple policies per table.

## Execution Instructions

1. **Run STEP 1 first** - Drops all existing policies (safe, won't fail)
2. **Run STEP 2** - Enables RLS on all tables
3. **Run STEP 3** - Creates all policies (sections 3.1 through 3.10)
4. **Run STEP 4** - Grants permissions
5. **Run STEP 5** - Verify everything worked

You can run each step separately or copy the entire file and run all at once.
