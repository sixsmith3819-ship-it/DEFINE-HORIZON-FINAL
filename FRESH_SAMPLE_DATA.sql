-- ============================================================================
-- SAMPLE DATA FOR FRESH DATABASE
-- Run this AFTER running FRESH_DATABASE_SETUP.sql
-- ============================================================================

-- This script requires at least ONE admin user to exist
-- Make sure you have run FRESH_DATABASE_SETUP.sql first
...
BEGIN
  ...
  INSERT INTO public.customers (
    customer_name, phone_number, email, id_number, address, 
    status, customer_type, assigned_employee_id, created_by
  ) VALUES
  ...
  INSERT INTO public.products (
    product_name, category, brand, model, selling_price, cost_price, 
    quantity, low_stock_threshold, supplier, status, created_by, updated_by
  ) VALUES
  ...
  ('Welcome to Define Horizon BMS', 'We are excited to launch our new Business Management System. This platform will streamline all our operations including customer management, transactions, inventory, and analytics.', 'high', 'published', NOW(), NOW() + INTERVAL '30 days', admin_user_id),
  ('System Training Sessions', 'Mandatory training sessions for all employees will be held next week. Monday: Customer Management, Wednesday: Transaction Processing, Friday: Inventory & Reports.', 'high', 'published', NOW(), NOW() + INTERVAL '14 days', admin_user_id),
  ('New Commission Rates', 'Updated commission structure: Local transactions - 2.5%, International transactions - 3.5%. These rates apply to all transactions processed from today onwards.', 'medium', 'published', NOW(), NOW() + INTERVAL '60 days', admin_user_id),
  ('Holiday Schedule', 'Office closed December 24th to January 2nd. Emergency contact numbers will be shared via email. Have a wonderful festive season!', 'low', 'draft', NOW() + INTERVAL '7 days', NOW() + INTERVAL '45 days', admin_user_id);
